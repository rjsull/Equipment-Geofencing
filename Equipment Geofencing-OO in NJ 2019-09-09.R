...
SET @POLY= geography::STGeomFromText('POLYGON((-84.358 39.653, -82.348 37.649, 
      -85.348 36.658, -88.358 37.658, -84.358 39.653))', 4326) --Kentucky State Borders (Rough)
SET @STATE = 'NJ' -- Seach State
SET @MILES = 3000
...
WHERE
	...
	--and 
	--and 
	trc_branch NOT IN ('001','002','003','004','UNKNOWN')
	...
	ISNULL(((CONVERT(decimal,(trc_gps_longitude))/60)/60),0)*-1, 4326).STIntersects(@POLY) = 0*/
	and RIGHT(LEFT(trc_gps_desc, CHARINDEX(', ', trc_gps_desc)+3),2) = @STATE
ORDER BY 1")
...
write.table(Filtered_TractorLocation, 
            file="C:/Users/SullivanRy/Documents/NJOO_TractorLocations.csv", 
            row.names=F,
            col.names=T,
            sep=",",
            append=FALSE)
...
ORDER BY 1")
  ...
  htmlwidgets::saveWidget(trcmap, file = "C:/Users/SullivanRy/Documents/NJOO_TractorLocations.html")
  ###Read in the pings data 
  TrcPings <- read.csv("NJOO_TractorLocations.csv",header=TRUE)
  ...
  htmlwidgets::saveWidget(trclines, file = "C:/Users/SullivanRy/Documents/NJOO_TractorLocationsLines.html")
